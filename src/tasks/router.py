...
@router.put("/{task_id}", response_model=TaskResponseSchema , status_code=status.HTTP_204_NO_CONTENT)
def update_task(body: TaskSchema , task_id : int, db : session = Depends(get_db)):
    return controller.update_task(body , task_id , db )
@router.delete("/{task_id}", response_model=TaskResponseSchema , status_code=status.HTTP_204_NO_CONTENT)
